...
export const playerDropCategories: PlayerDropCategoriesType = {
    ...
    security: {
        label: 'Security',
        description:
            'Player kicked out of the server due to suspect behavior such as sending too many commands or losing connection to the Cfx.re backend services.',
...
export const playerDropExpectedCategories = Object.keys(playerDropCategories).filter(
    (catId) => playerDropCategories[catId].expected,
);
export const playerDropUnexpectedCategories = Object.keys(playerDropCategories).filter(
    (catId) => !playerDropCategories[catId].expected,
);
