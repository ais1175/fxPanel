...
export const dateToLocaleTimeString = (
    ...
    return time.toLocaleTimeString(window.txBrowserLocale, { hour, minute, second, hour12 });
};
 ...
 */
export const dateToLocaleDateString = (time: Date, dateStyle: 'full' | 'long' | 'medium' | 'short' = 'long') => {
    return time.toLocaleDateString(window.txBrowserLocale, { dateStyle });
};

/**
 * Converts a timestamp to a locale date string
 */
export const tsToLocaleDateString = (ts: number, dateStyle: 'full' | 'long' | 'medium' | 'short' = 'long') => {
...
export const dateToLocaleDateTimeString = (
    ...
    return time.toLocaleString(window.txBrowserLocale, { dateStyle, timeStyle });
};
...
export const convertRowDateTime = (ts: number) => {
    ...
    return date.toLocaleString(window.txBrowserLocale, {
