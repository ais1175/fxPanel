...
suite('getMinTickIntervalMarker', () => {
    ...
    it('should handle a real case', () => {
        ...
        const minTickInterval = 0.05; // 50 ms - svMain
        const result = getMinTickIntervalMarker(boundaries, minTickInterval);
        expect(result).toBe(0.05);
    });
    it('should return undefined when threshold falls between boundaries', () => {
        const boundaries = [
            0.001,
            0.002,
            0.004,
        ...
        const minTickInterval = 0.2; // falls between 0.150 and 0.250
        const result = getMinTickIntervalMarker(boundaries, minTickInterval);
        expect(result).toBeUndefined();
    });
});
...
suite('getBucketTicketsEstimatedTime', () => {
    ...
    it('should handle real values in boundaries array', () => {
        ...
        const expected = [
            0.0005, 0.0015, 0.003, 0.005, 0.007, 0.009, 0.0125, 0.0175, 0.025, 0.04, 0.06, 0.085, 0.125, 0.2, 0.3125,
        ];
...
suite('getServerStatsData', () => {
    const hourMs = 60 * 60 * 1000;
    const minuteMs = 60 * 1000;
    const now = new Date('2024-06-01T12:00:00Z').getTime();

    const makeSnap = (startOffset: number, endOffset: number, players: number) => ({
    ...
    const makeLifespan = (
        snaps: ReturnType<typeof makeSnap>[],
        opts?: Partial<PerfLifeSpanType>,
    ): PerfLifeSpanType => ({
    ...
    it('should return 0 median and undefined uptime with empty lifespans', () => {
        const result = getServerStatsData([], 24, 0);
        expect(result.medianPlayerCount).toBe(0);
        expect(result.uptimePct).toBeUndefined();
    });

    it('should return undefined uptime when apiDataAge is 0', () => {
        const lifespans = [makeLifespan([makeSnap(-2 * hourMs, -1 * hourMs, 10)])];
        const result = getServerStatsData(lifespans, 24, 0);
        expect(result.uptimePct).toBeUndefined();
    });

    it('should calculate 100% uptime when server was up the entire window', () => {
        const windowHours = 1;
        const snaps = [makeSnap(-1 * hourMs, 0, 10)];
        const lifespans = [makeLifespan(snaps)];
        const result = getServerStatsData(lifespans, windowHours, now);
        expect(result.uptimePct).toBeCloseTo(100, 0);
    });

    it('should calculate partial uptime correctly', () => {
        const windowHours = 2;
        // Server was only up for the last 1 hour of a 2 hour window
        const snaps = [makeSnap(-1 * hourMs, 0, 10)];
        const lifespans = [makeLifespan(snaps)];
        const result = getServerStatsData(lifespans, windowHours, now);
        expect(result.uptimePct).toBeCloseTo(50, 0);
    });

    it('should calculate median player count across all snaps', () => {
        const snaps = [
            makeSnap(-5 * hourMs, -4 * hourMs, 5),
            makeSnap(-4 * hourMs, -3 * hourMs, 10),
            makeSnap(-3 * hourMs, -2 * hourMs, 15),
            makeSnap(-2 * hourMs, -1 * hourMs, 20),
            makeSnap(-1 * hourMs, 0, 25),
        ];
        const lifespans = [makeLifespan(snaps)];
        const result = getServerStatsData(lifespans, 24, now);
        expect(result.medianPlayerCount).toBe(15);
    });

    it('should handle snap that partially overlaps the window start', () => {
        const windowHours = 1;
        // Snap starts 30min before window, ends at window start
        // So only 0ms of this snap is within the window... but ended at windowStart so 0 contribution
        // Snap from -30min to 0 is fully within the 1h window
        const snaps = [
            makeSnap(-90 * minuteMs, -60 * minuteMs, 5), // entirely before window
            makeSnap(-30 * minuteMs, 0, 10), // fully within window
        ];
        const lifespans = [makeLifespan(snaps)];
        const result = getServerStatsData(lifespans, windowHours, now);
        // window is last 1h, snap[-30m, 0] = 30min out of 60min = 50%
        expect(result.uptimePct).toBeCloseTo(50, 0);
        // both snaps counted for player median
        expect(result.medianPlayerCount).toBe(7.5);
    });

    it('should skip lifespans that ended before the window', () => {
        const windowHours = 1;
        const oldLifespan = makeLifespan([makeSnap(-10 * hourMs, -9 * hourMs, 100)]);
        const recentLifespan = makeLifespan([makeSnap(-30 * minuteMs, 0, 10)]);
        const result = getServerStatsData([oldLifespan, recentLifespan], windowHours, now);
        // only the recent lifespan counts
        expect(result.medianPlayerCount).toBe(10);
        expect(result.uptimePct).toBeCloseTo(50, 0);
    });

    it('should handle multiple lifespans within the window', () => {
        const windowHours = 2;
        const lifespan1 = makeLifespan([makeSnap(-2 * hourMs, -1.5 * hourMs, 10)]);
        const lifespan2 = makeLifespan([makeSnap(-1 * hourMs, -0.5 * hourMs, 20)]);
        const result = getServerStatsData([lifespan1, lifespan2], windowHours, now);
        expect(result.medianPlayerCount).toBe(15);
        // 30min + 30min = 1h out of 2h = 50%
        expect(result.uptimePct).toBeCloseTo(50, 0);
    });

    it('should cap uptime at 100%', () => {
        const windowHours = 1;
        // Two overlapping lifespans that together exceed the window
        const snaps1 = [makeSnap(-1 * hourMs, 0, 10)];
        const snaps2 = [makeSnap(-1 * hourMs, 0, 10)];
        const result = getServerStatsData([makeLifespan(snaps1), makeLifespan(snaps2)], windowHours, now);
        expect(result.uptimePct).toBe(100);
    });

    it('should clamp partial-overlap snap start to window start', () => {
        const windowHours = 1;
        // Snap starts 30min before window (1.5h ago), ends 30min into window (30min ago)
        const snaps = [makeSnap(-90 * minuteMs, -30 * minuteMs, 10)];
        const lifespans = [makeLifespan(snaps)];
        const result = getServerStatsData(lifespans, windowHours, now);
        // windowStart = now - 1h. Snap goes from -90min to -30min.
        // Within window: windowStart(-60min) to -30min = 30min. So 30/60 = 50%
        expect(result.uptimePct).toBeCloseTo(50, 0);
    });
});
