...
export default function drawDropsTimeline({
    ...
    //Clear SVG
    select(svgRef).selectAll('*').remove(); // FIXME: Clear SVG

    //Setup selectors
    const svg = select<SVGElement, TimelineDropsDatum>(svgRef);
    ...
    //Scales
    const timeScale = scaleTime([data.startDate, data.endDate], [0, drawableAreaWidth]);
    const countsScaleMaxDomain = data.maxDrops < 10 ? data.maxDrops + 1 : data.maxDrops;
    const countsScale = scaleLinear([0, countsScaleMaxDomain], [height - margins.bottom, margins.top]);
    ...
    const timeAxisGroup = chartGroup
        .append('g')
        .attr('transform', translate(0, drawableAreaHeight))
        .attr('class', 'time-axis')
        .call(timeAxis);

    const countsAxisTickValues = countsScaleMaxDomain <= 7 ? range(countsScaleMaxDomain + 1) : null;
    const countsAxis = axisLeft(countsScale)
    ...
    const drawCanvasTimeline = () => {
        ...
        for (const intervalData of data.log) {
            ...
            for (const [dropCategory, dropCount] of intervalData.drops) {
                ...
                const barHeightRnd = Math.max(Math.floor(barHeight), barHeight > 0.8 ? 1 : 0);
                ...
                ctx.fillRect(renderStartX, barBottomY, barWidth, -barHeightRnd);
                ctx.strokeStyle = playerDropCategories[dropCategory]?.border ?? 'black';
                ctx.strokeRect(renderStartX + 0.5, barBottomY + 0.5, barWidth - 1, -barHeightRnd - 1);
            }
    ...
    const mask = defs.append('mask').attr('id', maskElmntId);
    const maskArea = mask
        .append('rect')
        ...
        .attr('opacity', '0');
    const maskRect = mask.append('rect').attr('height', drawableAreaHeight).attr('fill', 'black');
    chartGroup
        .append('rect')
        .attr('width', drawableAreaWidth)
        .attr('height', drawableAreaHeight)
    ...
    if (rangeSelected) {
        console.log(
            'Upsream range selected:',
            rangeSelected.startDate.toISOString(),
            rangeSelected.endDate.toISOString(),
        );
        updateRangeRect(timeScale(rangeSelected.startDate), timeScale(rangeSelected.endDate));
    }

    //Find the closest data point for a given X value
    const timeBisector = bisector((interval: TimelineDropsDatum) => interval.startDate).center;
    ...
    const handleMouseUp = (pointerX: number) => {
        ...
        if (
            !rangeCrossedThreshold &&
            rangeStartData.datum.startDate.getTime() === datumFound.datum.startDate.getTime()
        ) {
            return clearRangeData(true);
        }

        console.log('PlayerDrops range selected:', rangeStartData.datum.startDate, datumFound.datum.startDate);
        clearTimeout(cursorRedrawTimeout);
    ...
    chartGroup
        ...
        .on('mousemove', function (event) {
            const [pointerXVal] = pointer(event);
            if (!isEventInCooldown) {
                isEventInCooldown = true;
                handleMouseMove(pointerXVal);
                setTimeout(() => {
        ...
        .on('mousedown', function (event) {
            const [pointerXVal] = pointer(event);
            handleMouseDown(pointerXVal);
        })
        .on('mouseup', function (event) {
            const [pointerXVal] = pointer(event);
            handleMouseUp(pointerXVal);
        });
