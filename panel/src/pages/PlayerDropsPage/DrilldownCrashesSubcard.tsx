...
function CrashTypeRow({ datum, isLast, isOdd }: CrashTypeRowProps) {
    ...
    return (
        ...
        >
            ...
            <td className="min-w-[4ch] border-r px-2 py-1 text-right" title="Crash count">
                {datum.cntStr ?? '--'}
            </td>
            <td className="attempt-word-wrap line-clamp-4 px-2 py-1">{dataCellNode}</td>
        </tr>
    );
}
                    ...
                    {crashesData.filteredOut ? (
                        <tr>
                            ...
                            >
                                ...
                                {numberToLocaleString(crashTypes.length)} reasons which account for{' '}
                                {crashesData.displayPct} of all crashes. <br />
                                The remaining {numberToLocaleString(crashesData.filteredOut.types)} reasons account for{' '}
                                {crashesData.filteredOut.countPct} of all crashes.{' '}
                                <button
