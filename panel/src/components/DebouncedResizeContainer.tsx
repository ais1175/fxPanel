 ...
 */
function DebouncedResizeContainerInner({ delay, onDebouncedResize, children }: DebouncedResizeContainerProps) {
    ...
    return (
        ...
        >
            <div ref={loaderRef} className="absolute inset-0 flex items-center justify-center">
                <Loader2Icon className="text-muted-foreground size-16 animate-spin" />
            </div>
            <div className="absolute inset-0" ref={childRef}>
                {children}
            </div>
        </div>
    );
