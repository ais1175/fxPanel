...
export function DndSortableItem({ id, disabled, className, children }: DndSortableItemProps) {
    ...
    return (
        <li
            ...
            {...attributes}
            className={cn(
                'bg-card relative flex gap-3 rounded-lg border px-2 py-3 aria-pressed:z-50 aria-pressed:opacity-85 aria-pressed:shadow-xl',
                className,
            )}
        >
            <div
                ...
                className={cn(
                    ...
                    disabled ? 'cursor-not-allowed opacity-50' : 'hover:text-primary cursor-grab hover:scale-110',
                )}
            >
    ...
    return (
        ...
        >
            <SortableContext items={ids} strategy={verticalListSortingStrategy}>
                <ol ref={autoAnimateParentRef} className={className}>
                    {children}
                </ol>
            </SortableContext>
        </DndContext>
