...
export const PlayersListGrid: React.FC = () => {
    ...
    useEffect(() => {
        ...
        setBucket((prevBucketState) => {
            const highestPotentialBucket = Math.ceil(filteredPlayers.length / MAX_PER_BUCKET);
            // if our greatest possible bucket point in the new updated list,
            if (highestPotentialBucket < prevBucketState) return highestPotentialBucket;
            else return prevBucketState;
        });
    }, [filteredPlayers]);

    const slicedPlayers = useMemo(() => filteredPlayers.slice(0, MAX_PER_BUCKET * bucket), [filteredPlayers, bucket]);
    ...
    const handleObserver = useCallback(
        (entities: IntersectionObserverEntry[]) => {
            ...
            if (lastEntry.isIntersecting && filteredPlayers.length > slicedPlayers.length && !fakeLoading) {
                setFakeLoading(true);
                setTimeout(() => {
    ...
    return (
        <DivWrapper>
            <Box display="grid" gridTemplateColumns="repeat(auto-fill, minmax(300px, 1fr))">
