...
import { Autocomplete, Box, Button, DialogContent, MenuItem, TextField, Typography } from '@mui/material';
...
const DialogBanView: React.FC = () => {
    ...
    const handleBan = (e: React.FormEvent) => {
        ...
        fetchWebPipe<GenericApiResp>(`/player/ban?mutex=current&netid=${assocPlayer.id}`, {
            method: 'POST',
            data: {
            ...
            <form onSubmit={handleBan}>
                <Autocomplete
                    ...
                    renderOption={(props, option, state) => {
                        ...
                        const reason =
                            option.label.length > maxReasonSize
                        ...
                        return (
                            <li {...props} key={option.id}>
                                ...
                                >
                                    {banDurationToShortString(duration as any)}
                                </span>{' '}
                                {reason}
                            </li>
                        );
                ...
                <Button variant="contained" type="submit" color="error" sx={{ mt: 2 }} onClick={handleBan}>
