...
export const ensureProfileStructure = () => {
    const dataPath = txEnv.profileSubPath('data');
    if (!fs.existsSync(dataPath)) {
        fs.mkdirSync(dataPath);
    }

    const logsPath = txEnv.profileSubPath('logs');
...
export const setupProfile = () => {
    ...
    try {
        ...
        fs.writeFileSync(txEnv.profileSubPath('config.json'), JSON.stringify(configStructure, null, 2));
        ensureProfileStructure();
    } catch (error) {
        fatalError.Boot(4, ['Failed to set up data folder structure.', ['Path', txEnv.profilePath]], error);
    }
    ...
    if (txEnv.isWindows && txEnv.profileName !== 'default') {
        ...
        try {
            ...
            const batLines = [
                `@echo off`,
                `REM NOTE: Do not add any server convars in this file`,
