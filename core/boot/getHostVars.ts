...
export const hostEnvVarSchemas = {
    ...
    DATA_PATH: z
        .string()
        .min(1)
        .refine((val) => path.isAbsolute(val), 'DATA_PATH must be an absolute path'),
    ...
    TXA_PORT: z.coerce
        ...
        .int()
        .positive()
        .refine((val) => val !== 30120, 'TXA_PORT cannot be 30120'),
    FXS_PORT: z.coerce
        .number()
        .int()
        .positive()
    ...
    PROVIDER_NAME: z
        ...
        .refine((x) => !/[_.\- ]{2,}/.test(x), 'Provider name cannot contain consecutive special characters.')
        .refine(
    ...
    DEFAULT_ACCOUNT: z.string().refine((val) => {
