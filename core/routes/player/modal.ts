...
import { computePlayerTags, computePlayerTagsGeneric, getTagDefinitions } from '@lib/player/playerTags';
    ...
    try {
        return hist.map((log): PlayerHistoryItem => {
            return {
                ...
                author: log.author,
                ts: log.timestamp,
                exp: 'expiration' in log && log.expiration ? log.expiration : undefined,
                revokedBy: log.revocation?.author,
                revokedAt: log.revocation?.timestamp,
                revokedReason: log.revocation?.reason,
            };
        });
    } catch (error) {
        console.error(`Error processing player history: ${emsg(error)}`);
        return [];
    }
};
...
export default async function PlayerModal(ctx: AuthedCtx) {
    ...
    const playerData: PlayerModalPlayerData = {
        ...
        tags: player instanceof ServerPlayer ? computePlayerTags(player) : computePlayerTagsGeneric(player),
    };
    ...
    if (playerDbData) {
        ...
        playerData.nameHistory = playerDbData.nameHistory;
        playerData.sessionHistory = playerDbData.sessionHistory;
