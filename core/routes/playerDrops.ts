...
export type PlayerDropsDetailedWindow = Omit<PDLHourlyRawType, 'hour' | 'sessionTimeSum' | 'sessionTimeCount'> & {
...
export default async function playerDrops(ctx: AuthedCtx) {
    ...
    if (detailedWindow) {
        try {
            ...
            detailedWindowEnd = Math.min(lookupTs, new Date(windowEndStr).getTime());
        } catch (error) {
            return sendTypedResp({
                fail_reason: `Invalid date format: ${emsg(error)}`,
            });
        }
    } else {
