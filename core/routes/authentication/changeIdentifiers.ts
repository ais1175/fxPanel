...
import { changeIdentifiersBodySchema as bodySchema } from '@shared/authApiSchemas';
...
export default async function AuthChangeIdentifiers(ctx: AuthedCtx) {
    ...
    if (cfxreId.length) {
        try {
            ...
            return ctx.send<GenericApiResp>({
                error: `Failed to resolve CitizenFX ID to game identifier with error: ${emsg(error)}`,
            });
        }
    }
