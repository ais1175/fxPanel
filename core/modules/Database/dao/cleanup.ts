...
export default class CleanupDao {
    ...
    bulkRemove(
        ...
        try {
            ...
            const msg = `Failed to clean database with error: ${emsg(error)}`;
     ...
     */
    wipeHwids(fromPlayers: boolean, fromBans: boolean): number {
        ...
        if (typeof fromPlayers !== 'boolean' || typeof fromBans !== 'boolean')
            throw new Error('The parameters should be booleans.');
    ...
    runDailyOptimizer() {
        ...
        if (reportsRemoved) {
            console.ok(`- ${reportsRemoved} resolved reports past the retention period.`);
        }
    }
}
