...
import type { AdminStatsEntry } from '@shared/adminApiTypes';
...
export default class StatsDao {
    ...
    /**
     * Returns per-admin action stats (bans, warns, kicks, revocations)
     */
    getAdminStats(): Record<string, AdminStatsEntry> {
        const result: Record<string, AdminStatsEntry> = {};

        const ensure = (author: string) => {
            if (!result[author]) {
                result[author] = {
                    totalBans: 0,
                    totalWarns: 0,
        ...
        this.chain
            .get('actions')
            .forEach((action) => {
                const stats = ensure(action.author);
                stats.totalActions++;

                if (action.revocation) {
                    stats.revokedActions++;
                }
        ...
        // Count resolved tickets per admin
        this.chain
            .get('reports')
            .forEach((report) => {
                if (report.resolvedBy) {
                    const stats = ensure(report.resolvedBy);
                    stats.totalTicketsResolved++;
                }
            })
            .value();
    ...
    /**
     * Returns recent actions by a specific admin, newest first
     */
    getAdminRecentActions(adminName: string) {
        return this.chain
            .get('actions')
            .filter((a) => a.author === adminName)
            .sortBy('timestamp')
            .reverse()
            .map((a) => {
                const license = a.ids.find((id) => id.startsWith('license:'));
                return {
                    id: a.id,
                    type: a.type as 'ban' | 'warn' | 'kick',
                    playerName: a.playerName,
                    playerLicense: license ?? null,
                    reason: a.reason,
                    timestamp: a.timestamp,
                    isRevoked: !!a.revocation,
                };
            })
            .value();
    }
