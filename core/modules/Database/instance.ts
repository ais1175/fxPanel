...
//Consts & helpers
export { DATABASE_VERSION, defaultDatabase };
...
class JSONFile<T> implements Adapter<T> {
    ...
    constructor(filename: string) {
        ...
        this.serializer = txDevEnv.ENABLED ? (obj: any) => JSON.stringify(obj, null, 4) : JSON.stringify;
    }
...
export class DbInstance {
    ...
    async setupDatabase() {
        ...
        try {
            ...
            try {
                ...
                fatalError.Database(0, [
                    ...
                    ['Main error', emsg(errorMain)],
                    ['Backup error', emsg(errorBackup)],
        ...
        try {
            ...
            if (dbo.data.version !== DATABASE_VERSION) {
                await this.backupDatabase(txEnv.profileSubPath('data', `playersDB.backup.v${dbo.data.version}.json`));
    ...
    private async checkWriteNeeded() {
        ...
        if (this.#writePending === SavePriority.HIGH || sinceLastWrite > SAVE_CONFIG[this.#writePending].interval) {
            ...
            txManager.txRuntime.databaseSaveTime.count(timeElapsed.milliseconds);
        }
    }
    ...
    private async writeDatabase() {
        try {
            ...
            console.error(`Failed to save players database with error: ${emsg(error)}`);
            console.verbose.dir(error);
        }
    }
}
