...
const printDiagnostics = async () => {
    ...
    console.error(`fxPanel: ${txEnv.txaVersion}`);
 ...
 */
export const genActionID = (storage: IdStorageTypes, actionType: string): ActionId => {
    let attempts = 0;
    while (attempts < maxAttempts) {
        attempts++;
        const randFunc = attempts <= 5 ? nanoidSecure : nanoidNonSecure;
...
/**
 * Generates an unique report ID, or throws an error
 */
export const genReportID = (storage: IdStorageTypes) => {
    let attempts = 0;
    while (attempts < maxAttempts) {
        attempts++;
        const randFunc = attempts <= 5 ? nanoidSecure : nanoidNonSecure;
