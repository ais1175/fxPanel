...
export default class CacheStore {
    static readonly configKeysWatched = ['server.dataPath', 'server.cfgPath'];

    private cache: CacheMap = new Map();
    readonly cacheFilePath = txEnv.profileSubPath('data', CACHE_FILE_NAME);
    readonly throttledSaveCache = throttle(this.saveCache.bind(this), 5000, { leading: false, trailing: true });
    ...
    /**
     * Flush pending throttled save to disk on shutdown
     */
    public handleShutdown() {
        this.throttledSaveCache.cancel();
        this.saveCache();
    }
    ...
    private async saveCache() {
        try {
            const serializer = txDevEnv.ENABLED ? (obj: any) => JSON.stringify(obj, null, 4) : JSON.stringify;
            ...
            console.error(`Unable to save ${CACHE_FILE_NAME} with error: ${emsg(error)}`);
        }
    }
    ...
    private async loadCachedData() {
        try {
            ...
            if ((error as NodeJS.ErrnoException)?.code === 'ENOENT') {
            ...
            } else if ((error as Error)?.message === 'data_is_not_an_array') {
                ...
                console.warn(`Failed to load ${CACHE_FILE_NAME} with message: ${emsg(error)}`);
                console.warn('Since this is not a critical file, it will be reset.');
            }
        }
    }
