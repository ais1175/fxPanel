...
export default class ProcessManager {
    ...
    constructor(fxs: ValidChildProcess, props: ChildStateProps) {
        ...
        this.fxs.once('exit', (code, signal) => {
            ...
            if (this.tsExit - this.tsStart <= 5000) {
                console.defer(500).warn("FXServer didn't start. This is not an issue with fxPanel.");
            }
        });
        ...
        //Track I/O stats
        if (this.fxs.stdout) {
            this.fxs.stdout.on('data', (chunk: Buffer) => {
                this.stdoutBytes += chunk.length;
            });
        }
        if (this.fxs.stderr) {
            this.fxs.stderr.on('data', (chunk: Buffer) => {
    ...
    public get uptime() {
        const now = Date.now();
        return Math.min(this.tsKill ?? now, this.tsExit ?? now, this.tsClose ?? now) - this.tsStart;
    }
    ...
    /**
     * Track bytes written to stdin
     */
    public writeStdin(data: string) {
        if (!this.fxs?.stdin?.writable) return false;
        this.stdinBytes += Buffer.byteLength(data);
        return this.fxs.stdin.write(data);
    }
}
