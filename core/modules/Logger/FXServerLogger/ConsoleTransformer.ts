...
export default class ConsoleTransformer {
    ...
    public process(type: ConsoleLineEnum, data: string, context?: string): MultiBuffer {
        ...
        if (!this.lastEol) {
            ...
            if (src !== this.lastSrc) {
                ...
                fileBuffer += this.getFileTimestamp() + prefixed.fileBuffer;
            ...
            if (style === null) {
                ...
                webBuffer += colorLines(parts.first, style.web.line);
                stdoutBuffer += style.stdout ? colorLines(parts.first, style.stdout.line) : '';
            }
            if (parts.rest) {
    ...
    private prefixChunk(type: ConsoleLineEnum, chunk: string, context?: string): MultiBuffer {
        ...
        const webBuffer = colorLines(prefixMultiline(chunk, webPrefix + ' '), style.web.line);

        let stdoutBuffer = '';
        if (style.stdout) {
            const stdoutPrefix = style.stdout.prefix ? style.stdout.prefix(prefix) : prefix;
            stdoutBuffer = colorLines(prefixMultiline(chunk, stdoutPrefix + ' '), style.stdout.line);
        }
