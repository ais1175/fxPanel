...
const handleMemberSubcommand = async (interaction: ChatInputCommandInteraction, adminName: string) => {
    ...
    try {
        ...
        return await interaction.reply(embedder.danger(`Failed to save whitelist approval: ${emsg(error)}`));
    }
...
const handleRequestSubcommand = async (interaction: ChatInputCommandInteraction, adminName: string) => {
    ...
    try {
        ...
        if (!(error instanceof DuplicateKeyError)) {
            return await interaction.reply(embedder.danger(`Failed to save wl approval: ${emsg(error)}`));
        }
    }
    ...
    try {
        ...
        return await interaction.reply(embedder.danger(`Failed to remove wl request: ${emsg(error)}`));
    }
