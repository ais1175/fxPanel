...
If you just tried to edit a placeholder like \`{{serverBrowserUrl}}\` or \`{{serverJoinUrl}}\`, remember that those placeholders are replaced automatically by fxPanel, meaning you do not need to edit them at all.`;
...
export const generateStatusMessage = (
    ...
    try {
        ...
        throw new Error(`Embed JSON Error: ${emsg(error)}`);
    }

    let embedConfigJson;
    try {
        embedConfigJson = jsonForgivingParse(rawEmbedConfigJson);
        if (!(embedConfigJson instanceof Object)) throw new Error(`not an Object`);
    } catch (error) {
        throw new Error(`Embed Config JSON Error: ${emsg(error)}`);
    }
    ...
    function processObject(inputData: object) {
        const input = structuredClone(inputData);
        const out: any = {};
        for (const [key, value] of Object.entries(input)) {
            const processed = processValue(value);
            if (key === 'url' && !isValidEmbedUrl(processed)) {
                const messageHead = processed.length ? `Invalid URL \`${processed}\`.` : `Empty URL.`;
                const badPlaceholderMessage = processed.startsWith('{{') ? invalidPlaceholderMessage : '';
                throw new Error([messageHead, invalidUrlMessage, badPlaceholderMessage].join('\n'));
            }
            out[key] = processed;
        }
        return out;
    ...
    try {
        ...
        embed.setTimestamp();
        if (!processedEmbedData.footer) {
            embed.setFooter({
                iconURL: 'https://cdn.discordapp.com/emojis/1062339910654246964.webp?size=96&quality=lossless',
                text: `fxPanel ${txEnv.txaVersion} • Updated every minute`,
            });
        }
    } catch (error) {
        throw new Error(`**Embed Class Error:** ${emsg(error)}`);
    }
    ...
    try {
        if (Array.isArray(embedConfigJson?.buttons) && embedConfigJson.buttons.length) {
            ...
            for (const cfgButton of embedConfigJson.buttons) {
                ...
                if (!isValidEmbedUrl(processedUrl)) {
                    const messageHead = processedUrl.length ? `Invalid URL \`${processedUrl}\`` : `Empty URL`;
                    const badPlaceholderMessage = processedUrl.startsWith('{{') ? invalidPlaceholderMessage : '';
                    throw new Error(
                        [
        ...
        throw new Error(`**Embed Buttons Error:** ${emsg(error)}`);
    }
...
export default async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
        ...
        return await interaction.reply(embedder.warning(`**Failed to generate new embed:**\n${emsg(error)}`, true));
    }
    ...
    try {
        ...
        return await interaction.reply(embedder.warning(`**Failed to send new embed:**\n${msg}`, true));
    }
