...
export const ensurePermission = async (interaction: CommandInteraction, reqPerm: string) => {
    ...
    if (!admin) {
        await interaction.reply(
            embedder.warning(
                `**Your account does not have fxPanel access.** :face_with_monocle:\nIf you are already registered in fxPanel, visit the Admin Manager page, and make sure the Discord ID for your user is set to \`${interaction.user.id}\`.`,
                true,
            ),
        );
        return false;
    ...
    if (
        ...
        await interaction.reply(
            embedder.danger(
                `Your fxPanel account does not have the "${permName}" permissions required for this action.`,
                true,
            ),
        );
        return false;
...
export const logDiscordAdminAction = async (adminName: string, message: string) => {
    txCore.logger.system.write(adminName, message, 'action');
};
