...
export const getScopeDefaults = <T>(scope: ConfigScope): T => {
    return Object.fromEntries(Object.entries(scope).map(([key, schema]) => [key, schema.default])) as T;
};
...
const attemptConfigFix = (scope: string, key: string, value: any, configSchema: ScopeConfigItem) => {
    ...
    if (configSchema.fixer === SYM_FIXER_DEFAULT) {
        ...
        try {
            ...
            console.error(`Invalid value for '${scope}.${key}', fixer failed with reason: ${emsg(error)}`);
            return {
                success: false,
                error,
            };
...
export const bootstrapConfigProcessor = (
    ...
    const active = structuredClone(defaultConfigs);
    ...
    for (const { scope, key, value } of parsedInput) {
        ...
        if (fResult.success && fResult.value !== undefined) {
            ...
            console.warn(`Invalid value for '${scope}.${key}': ${zResult.error.message}`);
...
export const runtimeConfigProcessor = (
    ...
    const thisStoredCopy = structuredClone(storedConfigs);
    const thisActiveCopy = structuredClone(activeConfigs);
