...
export default class FxMonitor {
    ...
    private updateStatus() {
        ...
        if (result.action === 'RESTART') {
            ...
            txManager.txRuntime.registerFxserverRestart(result.cause);
        }
    }
    ...
    private calculateMonitorStatus(): ProcessStatusResult {
        ...
        const currentStatusString =
            heartBeat.state !== MonitorState.HEALTHY && healthCheck.state !== MonitorState.HEALTHY
                ? FxMonitorHealth.OFFLINE
                : FxMonitorHealth.PARTIAL;
        ...
        if (healthCheck.state !== MonitorState.HEALTHY) {
            ...
            if (healthCheck.state === MonitorState.PENDING) {
                ...
                if (heartBeat.state !== MonitorState.PENDING) {
                    healthCheckIssue.addInfo(
                        `Resources started ${secs(heartBeat.secsSinceFirst)} ago but the HTTP endpoint is still unresponsive.`,
                    );
        ...
        if (heartBeat.state !== MonitorState.HEALTHY) {
            ...
            if (heartBeat.state === MonitorState.PENDING) {
                ...
                if (resBoot.current?.time.isOver(txConfig.restarter.resourceStartingTolerance)) {
                    ...
                    heartBeatIssue.addInfo(
                        `Resource "${resBoot.current.name}" has been loading for ${secs(resBoot.current.time.elapsed)}.`,
                    );
                    ...
                    if (resBoot.elapsedSinceLast === null) {
                        ...
                        if (processUptime > HB_CONFIG.bootNoEventLimit) {
                            heartBeatIssue.addInfo(
                                `No resource event received within the ${secs(HB_CONFIG.bootNoEventLimit)} limit.`,
                            );
                            return {
                                action: 'RESTART',
                                times: timeTags.withProc,
    ...
    private async performHealthCheck() {
        ...
        if (dynamicJson.sv_maxclients) {
            ...
            if (txHostConfig.forceMaxClients && maxClients > txHostConfig.forceMaxClients) {
                ...
                console.error(
                    `${txHostConfig.sourceName}: Detected that the server has sv_maxclients above the limit (${txHostConfig.forceMaxClients}). Changing back to the limit.`,
                );
                txCore.logger.system.writeSystem(
                    'SYSTEM',
    ...
    public handleHeartBeat(source: 'fd3' | 'http') {
        ...
        if (source === 'fd3') {
            ...
            if (this.swLastFD3.started && this.swLastFD3.elapsed > 15 && this.swLastHTTP.elapsed < 5) {
                txManager.txRuntime.registerFxserverHealthIssue('fd3');
            }
    ...
    private async handleBootCompleted(bootDuration: number) {
        ...
        txManager.txRuntime.registerFxserverBoot(bootDuration);
    ...
    public get status() {
        ...
        const lastEntry = this.statusLog.at(-1);
        if (lastEntry) {
            const healthReasonLines = [lastEntry.reason + '.'];
