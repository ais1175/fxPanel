...
export const localeFileSchema = z
    .object({
        $meta: z.object({
...
export default class Translator {
    ...
    getLanguagePhrases(lang: string) {
        if (localeMap[lang]?.$meta) {
            ...
            try {
                return JSON.parse(fs.readFileSync(this.customLocalePath, 'utf8'));
            } catch (error) {
                throw new Error(`Failed to load '${this.customLocalePath}'. (${emsg(error)})`);
            }
        } else {
