...
import {
    ...
    PDLHourlyRawType,
    PDLHourlyType,
    PDLServerBootDataSchema,
} from './playerDropSchemas';
...
import { LOG_DATA_FILE_VERSION } from './consts';
const console = consoleFactory(modulename);

//Consts
export { LOG_DATA_FILE_VERSION };
...
export default class PlayerDropMetrics {
    ...
    private queueSaveEventLog = throttle(15_000, this.saveEventLog.bind(this), { noLeading: true });
    ...
    public getWindowData(windowStart: number, windowEnd: number): PlayerDropsDetailedWindow {
        ...
        for (const log of filteredLogs) {
            ...
            sessionTimeSum += log.sessionTimeSum;
            sessionTimeCount += log.sessionTimeCount;
        }
        return {
            changes: allChanges,
            ...
            avgSessionSeconds: sessionTimeCount > 0 ? Math.round(sessionTimeSum / sessionTimeCount) : null,
        };
    }
    ...
    public handlePlayerDrop(event: PlayerDropEvent) {
        ...
        if (event.sessionTimeSeconds && event.sessionTimeSeconds > 0) {
            logRef.sessionTimeSum += event.sessionTimeSeconds;
            logRef.sessionTimeCount++;
        }
        ...
        try {
            ...
            if (fileData.version === LOG_DATA_FILE_VERSION) {
                ...
                try {
                    ...
                    throw new Error(
                        `Failed to migrate ${LOG_DATA_FILE_NAME} from ${fileData?.version} to ${LOG_DATA_FILE_VERSION}: ${emsg(error)}`,
                    );
                }
            }
            ...
            this.eventLog = statsData.log.map((entry): PDLHourlyType => {
                return {
                    ...
                    sessionTimeSum: entry.sessionTimeSum,
                    sessionTimeCount: entry.sessionTimeCount,
                };
            });
