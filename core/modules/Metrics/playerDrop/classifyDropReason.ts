...
const cleanCrashReason = (reason: string) => {
    ...
    const saveMsg = exceptionPrefix ? 'Unhandled exception: ' + msg.slice(exceptionPrefix.length) : msg;
...
const guessDropReasonCategory = (reason: string): ClassifyDropReasonResponse => {
    ...
    } else if (serverShutdownRules.some((rule) => reasonToMatch.startsWith(rule))) {
...
export const classifyDrop = (payload: PlayerDropEvent): ClassifyDropReasonResponse => {
    ...
    if (typeof payload.category !== 'number' || payload.category <= 0) {
        return {
            category: 'unknown',
            cleanReason: truncateReason(payload.reason, PDL_UNKNOWN_REASON_CHAR_LIMIT, '[tx:invalid-category]'),
        };
        ...
        return {
            category: 'unknown',
            cleanReason: truncateReason(payload.reason, PDL_UNKNOWN_REASON_CHAR_LIMIT, '[tx:unknown-category]'),
        };
    }
};
