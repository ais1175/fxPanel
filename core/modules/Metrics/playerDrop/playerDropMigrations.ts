...
export const migratePlayerDropsFile = async (fileData: any): Promise<PDLFileType> => {
    ...
    if (fileData.version === 1) {
        ...
        for (const log of data.log) {
            ...
            log.crashTypes = log.crashTypes.map(([reason, count]) => {
                const newReason = reason.startsWith(crashPrefix) ? reason.slice(crashPrefix.length) : reason;
                return [newReason, count];
            });
            //@ts-expect-error v1 migration: dropTypes had different shape
            log.dropTypes = log.dropTypes
                .map(([type, count]): [string, number] | false => {
            ...
            //@ts-expect-error v1 migration: resKicks did not exist in v1
            log.resKicks = [];
        }
    ...
    if (fileData.version === 2) {
        console.warn('Migrating your player drops stats v2 to v3.');
        const data = PDLFileSchema_v2.parse(fileData);
        for (const log of data.log) {
            (log as any).sessionTimeSum = 0;
            (log as any).sessionTimeCount = 0;
        }
