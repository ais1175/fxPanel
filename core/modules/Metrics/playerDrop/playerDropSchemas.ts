...
export const PDLHourlyRawSchema = z.object({
    hour: z.string(),
    changes: z.array(z.union([PDLFxsChangedEventSchema, PDLGameChangedEventSchema, PDLResourcesChangedEventSchema])),
    ...
    sessionTimeSum: z.number().nonnegative().default(0),
    sessionTimeCount: z.number().int().nonnegative().default(0),
});
...
export const PDLHourlyRawSchema_v1 = PDLHourlyRawSchema.extend({
    changes: z.array(
        z.union([PDLFxsChangedEventSchema_v1, PDLGameChangedEventSchema_v1, PDLResourcesChangedEventSchema]),
    ),
}).omit({
    resKicks: true,
});
...
/**
 * Migration schemas from v2 to v3 with changes:
 * - added sessionTimeSum and sessionTimeCount to hourly entries
 */
export const PDLHourlyRawSchema_v2 = PDLHourlyRawSchema.omit({
    sessionTimeSum: true,
    sessionTimeCount: true,
});
export const PDLFileSchema_v2 = PDLFileSchema.extend({
    version: z.literal(2),
    log: z.array(PDLHourlyRawSchema_v2),
});
export type PDLFileType_v2 = z.infer<typeof PDLFileSchema_v2>;
