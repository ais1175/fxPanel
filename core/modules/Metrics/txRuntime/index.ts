...
export default class TxRuntimeMetrics {
    ...
    public resourceRuntimes: Record<string, number> | undefined;
    ...
    /**
     * Called via FD3 to store the resource runtime usage counts
     */
    handleResourceRuntimes(payload: { runtimes: Record<string, number> }) {
        if (typeof payload?.runtimes !== 'object' || payload.runtimes === null) return;
        this.resourceRuntimes = payload.runtimes;
    }
    ...
    async refreshHbData() {
        ...
        try {
            ...
            const statsData = {
                ...
                isPterodactyl: !txEnv.isWindows && process.env?.TXADMIN_ENABLE === '1',
            ...
            this.currHbData = JSON.stringify({ error: emsg(error) });
        }
    }
}
