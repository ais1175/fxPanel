...
import { MultipleCounter, QuantileArray, TimeCounter, estimateArrayJsonSize, isWithinMargin } from './statsUtils';
...
suite('MultipleCounter', () => {
    ...
    it('should instantiate with object correctly', () => {
        expect(counterWithData.toArray()).toEqual([
            ['a', 1],
            ['b', 2],
        ]);
    ...
    it('should sort the data', () => {
        ...
        expect(counter.toSortedKeysArray()).toEqual([
            ['a', 3],
            ['c', 2],
            ['z', 1],
        ]);
        expect(counter.toSortedKeysArray(true)).toEqual([
            ['z', 1],
            ['c', 2],
            ['a', 3],
        ]);
        expect(counter.toSortedValuesArray()).toEqual([
            ['z', 1],
            ['c', 2],
            ['a', 3],
        ]);
        expect(counter.toSortedValuesArray(true)).toEqual([
            ['a', 3],
            ['c', 2],
            ['z', 1],
        ]);
