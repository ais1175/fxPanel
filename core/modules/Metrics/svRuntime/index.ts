...
import type {
    ...
    SvRtLogType,
    SvRtNodeMemoryType,
    SvRtPerfBoundariesType,
    SvRtPerfCountsType,
} from './perfSchemas';
import { didPerfReset, diffPerfs, fetchFxsMemory, fetchRawPerfData, getPromAuthOpts } from './perfUtils';
...
export default class SvRuntimeMetrics {
    private readonly logFilePath = txEnv.profileSubPath('data', LOG_DATA_FILE_NAME);
    ...
    private lastRawPerfSaved:
        ...
        | undefined;
    private queueSaveStatsHistory = throttle(15_000, this.saveStatsHistory.bind(this), { noLeading: true });
    ...
    private async collectStats() {
        ...
        txManager.txRuntime.perfCollectionTime.count(collectionTime.milliseconds);
        ...
        if (txDevEnv.EXT_STATS_HOST) {
            try {
                ...
                /* external stats unavailable, use local count */
            }
        }
    ...
    private async loadStatsHistory() {
        try {
            ...
            if (error instanceof ZodError) {
                ...
                console.warn(`Failed to load ${LOG_DATA_FILE_NAME} with message: ${emsg(error)}`);
            }
    ...
    private async saveStatsHistory() {
        try {
            ...
            console.warn(`Failed to save ${LOG_DATA_FILE_NAME} with message: ${emsg(error)}`);
        }
    }

    /**
     * Returns the player count + memory time series for the insights page
     */
    public getPlayerCountSeries() {
        return this.statsLog.filter(isSvRtLogDataType).map((snap) => ({
            ts: snap.ts,
            players: snap.players,
            fxsMemory: snap.fxsMemory,
            nodeMemory: snap.nodeMemory,
        }));
    }

    /**
     * Returns the full stats log including boot/close events (for uptime timeline)
     */
    public getFullStatsLog() {
        return this.statsLog.map((entry) => {
            if (entry.type === 'data') {
                return { ts: entry.ts, type: entry.type as const, players: entry.players };
            }
            if (entry.type === 'svBoot') {
                return { ts: entry.ts, type: entry.type as const };
            }
            return { ts: entry.ts, type: entry.type as const, reason: entry.reason };
        });
    }
    ...
    /**
     * Flush pending throttled save to disk
     */
