...
class ScanLimitError extends Error {
    constructor(code: string, upperLimit: number, current: number) {
...
export type CompressionResult = {
    raw: Buffer;
    gz: Buffer;
    br?: Buffer; // Brotli compressed (optional for backwards compatibility)
};
type StaticFilePath = {
    url: string;
};
...
const compressGzip = async (buffer: Buffer) => {
    return gzip(buffer, { chunkSize: 64 * 1024, level: 6 });
};

const compressBrotli = async (buffer: Buffer) => {
...
export const getCompressedFile = async (fullPath: string): Promise<CompressionResult> => {
    const raw = await fsp.readFile(fullPath);
    const [gz, br] = await Promise.all([compressGzip(raw), compressBrotli(raw)]);
    return { raw, gz, br };
};

/**
 * Reads the Vite manifest from .vite/manifest.json to build a Set of valid output file URLs.
 * Falls back to allowing all files if the manifest is missing (e.g. dev mode).
 */
const loadPanelManifest = (rootPath: string): Set<string> | null => {
    try {
        const manifestPath = path.join(rootPath, '.vite', 'manifest.json');
        const raw = fs.readFileSync(manifestPath, 'utf-8');
        const manifest: Record<string, { file?: string; css?: string[] }> = JSON.parse(raw);
...
//Checks if a file URL should be served from a panel root
...
export const scanStaticFolder = async ({ rootPath, state, limits }: ScanFolderOpts) => {
    //Load manifest for panel roots to build the file whitelist
    const manifestFiles = rootPath.endsWith('panel') ? loadPanelManifest(rootPath) : null;
    ...
    while (foldersToScan.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (!checkFileWhitelist(manifestFiles, entryPathUrl)) {
...
const preProcessFiles = async (opts: ServeStaticMwOpts) => {
    ...
    const brSize = scanState.files.reduce((acc, file) => acc + (file.br?.length ?? 0), 0);
    // console.dir({
    //     rawSize: bytes(rawSize),
    //     gzSize: bytes(gzSize),
    //     gzPct: (gzSize / rawSize * 100).toFixed(2) + '%',
    //     brSize: bytes(brSize),
    //     brPct: (brSize / rawSize * 100).toFixed(2) + '%',
    // });
    return {
        files: scanState.files,
        memSize: scanState.bytes + gzSize + brSize,
    };
};
...
const serveStaticMwProd = (opts: ServeStaticMwOpts) => async (ctx: RawKoaCtx, next: Next) => {
    ...
    //Check encoding preference: brotli > gzip > raw
    const encoding = ctx.acceptsEncodings('br', 'gzip', 'identity');
    if (encoding === 'br' && staticFile.br) {
        ctx.set('Content-Encoding', 'br');
        ctx.body = staticFile.br;
