...
export default class FxScheduler {
    static readonly configKeysWatched = ['restarter.schedule', 'restarter.intervalHours'];
    ...
    private intervalAnchorTs: number = Date.now();
    ...
    constructor() {
        ...
        //Self-aligning minute-boundary check for precision
        this.scheduleNextCheck();
    }

    /**
     * Schedules the next check aligned to the start of each minute.
     * Uses self-scheduling setTimeout instead of setInterval to avoid drift.
     */
    private scheduleNextCheck() {
        const now = new Date();
        const delay = 60_000 - (now.getSeconds() * 1000 + now.getMilliseconds()) + 50;
        setTimeout(() => {
            this.checkSchedule();
    ...
    handleConfigUpdate(updatedConfigs: UpdateConfigKeySet) {
        ...
        if (updatedConfigs.has('restarter.intervalHours')) {
            this.intervalAnchorTs = Date.now();
        }
        this.checkSchedule();
        txCore.webServer.webSocket.pushRefresh('status');
    }
     ...
     * If the time is after the next config restart, the config restart will be postponed.
     */
    setNextTempSchedule(timeString: string) {
        ...
        if (timeString.startsWith('+')) {
            ...
            if (typeof hours === 'undefined' || isNaN(hours) || hours < 0 || hours > 23)
                throw new Error('invalid hours');
            if (typeof minutes === 'undefined' || isNaN(minutes) || minutes < 0 || minutes > 59)
                throw new Error('invalid minutes');
            ...
            if (scheduledMinuteFloorTs === thisMinuteTs) {
                throw new Error(
                    'Due to the 1 minute precision of the restart scheduler, you cannot schedule a restart in the same minute.',
                );
            }
            if (scheduledMinuteFloorTs < thisMinuteTs) {
        ...
        //If there's a config restart before this temp schedule, it will be skipped
        if (Array.isArray(txConfig.restarter.schedule) && txConfig.restarter.schedule.length) {
            ...
            if (nextSettingRestart.minuteFloorTs < scheduledMinuteFloorTs) {
                console.warn(
                    `Temporary restart at ${scheduledString} is after the next config restart at ${nextSettingRestart.string}. The config restart will be postponed.`,
                );
            }
        }
    ...
    async checkSchedule(calculateOnly = false) {
        ...
        let nextRestart: RestartInfo | undefined;
        if (this.nextTempSchedule) {
        ...
        //Check interval-based restart
        if (txConfig.restarter.intervalHours > 0 && !this.nextTempSchedule) {
            const intervalMs = txConfig.restarter.intervalHours * 60 * 60 * 1000;
            const nextIntervalTs = this.intervalAnchorTs + intervalMs;
            const nextIntervalMinuteFloorTs = new Date(nextIntervalTs).setSeconds(0, 0);
            const nextDate = new Date(nextIntervalMinuteFloorTs);
            const intervalTimeString =
                nextDate.getHours().toString().padStart(2, '0') +
                ':' +
                nextDate.getMinutes().toString().padStart(2, '0');
            const intervalRestart: RestartInfo = {
                string: intervalTimeString,
                minuteFloorTs: nextIntervalMinuteFloorTs,
            };
            if (!nextRestart || intervalRestart.minuteFloorTs < nextRestart.minuteFloorTs) {
                nextRestart = intervalRestart;
            }
        }
