...
class Command {
    ...
    isConvarSetterFor(varname: string) {
        ...
        if (this.command === varname.toLowerCase() && this.args.length === 1) {
            return this.args[0];
        }
    ...
    toMarkdown(hasHostConfig = false) {
        ...
        for (const file of files) {
            ...
            for (const [line, msg] of fileInfos) {
                ...
                const cleanedMsg = msg.replaceAll(/\n\t/gm, '\n');
                msgLines.push(`${linePrefix}${cleanedMsg}`);
            }
        }
        if (hasHostConfig) {
            msgLines.push('');
...
export const findLikelyCFGPath = (serverDataPath: string) => {
    const commonCfgFileNames = ['server.cfg', 'server.cfg.txt', 'server.cfg.cfg', 'server.txt', 'server'];
    ...
    for (const cfgFileName of commonCfgFileNames) {
        ...
        try {
            ...
            /* file not found, try next cfg filename */
        }
    }
    return false;
};
...
export const readRawCFGFile = async (cfgPath: string) => {
    ...
    if (!fs.lstatSync(cfgPath).isFile()) {
        throw new Error(
            "File doesn't exist or its unreadable. Make sure to include the CFG file in the path, and not just the directory that contains it.",
        );
    }
...
export const readLineCommands = (input: string) => {
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (!currToken.length) {
            if (input.slice(i, i + 2) === '//' || input[i] === '#') {
                break;
            }
        }

        //Treat ASCII control chars and Unicode whitespace as token separators
        const charCode = input.charCodeAt(i);
        if (!inQuote && (charCode <= 32 || /\s/.test(input[i]))) {
            if (currToken.length) {
                ...
                if (!cmdObject.args[0].startsWith('@')) {
                    ...
                    try {
                        const extractedCommands = await parseRecursiveConfig(
                            null,
                            recursiveCfgAbsolutePath,
                            serverDataPath,
                            stack,
                        );
                        parsedCommands.push(...extractedCommands);
                    } catch (error) {
                        parsedCommands.push(new ExecRecursionError(cfgAbsolutePath, emsg(error), lineNumber));
                    }
                }
            }
        }
...
const validateCommands = async (parsedCommands: (ExecRecursionError | Command)[]) => {
    ...
    const requiredGameName = txHostConfig.forceGameName
        ? txHostConfig.forceGameName === 'fivem'
            ? 'gta5'
            : 'rdr3'
        : undefined;
    ...
    for (const cmd of parsedCommands) {
        ...
        //Check for non-ASCII characters in command names (e.g. invisible Unicode spaces from copy-paste)
        if (/[^\x20-\x7E]/.test(cmd.command)) {
            errors.add(
                cmd.file,
                cmd.line,
                `the command \`${cmd.command.replace(/[^\x20-\x7E]/g, '?')}\` contains invisible or non-ASCII characters. This usually happens when copy-pasting from websites. Please retype the line manually.`,
            );
            continue;
        }
        ...
        if (txHostConfig.forceMaxClients && isMaxClientsString) {
            const maxClients = parseInt(isMaxClientsString);
            if (maxClients > txHostConfig.forceMaxClients) {
                hasHostConfigMessage = true;
                errors.add(cmd.file, cmd.line, `your 'sv_maxclients' MUST be <= ${txHostConfig.forceMaxClients}.`);
                continue;
            }
        }
        ...
        if (txHostConfig.forceGameName && isGameNameString) {
            detectedGameName = isGameNameString;
            if (isGameNameString !== requiredGameName) {
                hasHostConfigMessage = true;
                errors.add(cmd.file, cmd.line, `your 'gamename' MUST be '${requiredGameName}'.`);
                continue;
            }
        }
        ...
        if (cmd.command === 'endpoint_add_tcp' || cmd.command === 'endpoint_add_udp') {
            ...
            if (!Array.isArray(matches) || !matches.length) {
                errors.add(cmd.file, cmd.line, `the \`${cmd.args[0]}\` is not in a valid \`ip:port\` format.`);
                continue;
            }
            ...
            if (txHostConfig.fxsPort && port !== txHostConfig.fxsPort) {
                hasHostConfigMessage = true;
                errors.add(cmd.file, cmd.line, `the \`${cmd.command}\` port MUST be \`${txHostConfig.fxsPort}\`.`);
                continue;
            }
    ...
    if (txHostConfig.forceGameName === 'redm' && detectedGameName !== 'rdr3') {
        ...
        hasHostConfigMessage = true;
        errors.add(initFile, false, `your config MUST have a 'gamename' set to '${requiredGameName}'.`);
    }
...
const getConnectEndpoint = (endpoints: EndpointsObjectType, hasEndpointCommand: boolean) => {
    ...
    if (!tcpudpEndpoint) {
        throw new Error(
            'Your config file does not not contain a ip:port used in both `endpoint_add_tcp` and `endpoint_add_udp` commands. Players would not be able to connect.',
        );
    }
...
export const validateFixServerConfig = async (cfgPath: string, serverDataPath: string) => {
    ...
    const { endpoints, hasEndpointCommand, hasHostConfigMessage, errors, warnings, toCommentOut } =
    ...
    try {
        ...
        errors.add(cfgAbsolutePath, false, emsg(error));
    }
 ...
 */
export const validateModifyServerConfig = async (cfgInputString: string, cfgPath: string, serverDataPath: string) => {
    ...
    try {
        ...
        throw new Error(`Failed to edit 'server.cfg' with error: ${emsg(error)}`);
    }
