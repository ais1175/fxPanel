...
export default (mutex: any, netid: any, license: any) => {
    ...
    if (typeof mutex === 'string' && typeof netid === 'number' && !isNaN(parsedNetid)) {
        ...
        if (mutex && mutex === txCore.fxRunner.child?.mutex) {
            ...
            // If mutex is from previous server, search licenseCache
            // The psid stored in licenseCache may have a rollover suffix (e.g. "mutex#netidrN")
            const found = txCore.fxPlayerlist.licenseCache.find((c) => {
                return c[0] === `${mutex}#${netid}` || c[0].startsWith(`${mutex}#${netid}r`);
            });
