...
export const getPathContent = async (root: string, filter?: (entry: Dirent) => boolean) => {
    ...
    return allEntries.filter((entry) => ((entry.isFile() || entry.isDirectory()) && filter ? filter(entry) : true));
};
 ...
 * Equivalent to fs-extra's outputFile.
 */
...
/**
 * Moves a file or directory, falling back to copy+delete if rename fails (e.g. cross-device).
 * Equivalent to fs-extra's move.
 */
export const movePath = async (srcPath: string, destPath: string, overwrite = false) => {
    if (overwrite) {
    ...
    try {
        ...
        if (error instanceof Error && 'code' in error && error.code === 'EXDEV') {
            await fsp.cp(srcPath, destPath, { recursive: true, force: overwrite });
...
/**
 * Validates that a filename is safe to use and doesn't contain path traversal attempts.
