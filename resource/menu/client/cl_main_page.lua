...
RegisterCommand('goto', function(_, args)
    ...
    if #args < 1 then
        return SendSnackbarMessage('error', 'nui_menu.page_main.teleport.goto.usage', true)
    end
    local tgtId = tonumber(args[1])
    if not tgtId then
...
RegisterSecureNuiCallback('healRadius', function(data, cb)
    TriggerServerEvent('txsv:req:healRadius', tonumber(data.radius))
    cb({})
end)
...
RegisterNetEvent('txcl:clearArea', function(radius)
    SendSnackbarMessage('success', 'nui_menu.page_main.clear_area.dialog_success', true, { radius = radius })
    if IS_REDM then
        return
    end
