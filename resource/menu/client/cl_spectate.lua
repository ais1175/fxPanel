        ...
        while isSpectateEnabled and storedTargetServerId == initialTargetServerid do
            ...
            if not DoesEntityExist(storedTargetPed) then
                ...
                if newPed > 0 then
                    ...
                    SendSnackbarMessage(
                        'error',
                        'nui_menu.player_modal.actions.interaction.notifications.spectate_failed',
                        true
                    )
                    DebugPrint(
                        ('Spectated player (%s) no longer exists, ending spectate...'):format(storedTargetPlayerId)
                    )
                    stopSpectating()
                    break
                end
...
local function handleSpecCycle(isNext)
    ...
    DebugPrint(
        ('Cycling spectate from target: %s, isNext: %s'):format(tostring(storedTargetServerId), tostring(isNext))
    )
...
RegisterNetEvent('txcl:spectate:start', function(targetServerId, targetCoords)
    ...
    if targetServerId == GetPlayerServerId(PlayerId()) then
        return SendSnackbarMessage(
            'error',
            'nui_menu.player_modal.actions.interaction.notifications.spectate_yourself',
            true
        )
    end
