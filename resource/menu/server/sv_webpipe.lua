...
if not useLatentEvents then
    TxPrint(
        '^3WARNING: Latent events are disabled. If you have issues using the txAdmin in-game menu, please enable them by setting sv_enableNetEventReassembly to true in your server.cfg.'
    )
end
...
local function SendMenuMessage(src, callbackId, statusCode, path, body, headers, cached)
    local errorCode = tonumber(statusCode) >= 400
    local resultColor = errorCode and '^1' or '^2'
    local cachedStr = cached and ' ^1(cached)^0' or ''
    DebugWebPipe(
        ('^3WebPipe[^5%d^0:^1%d^3]^0 %s<< %s ^4%s%s^0'):format(
            src,
            callbackId,
            resultColor,
            statusCode,
    ...
    if errorCode then
        DebugWebPipe(
            ('^3WebPipe[^5%d^0:^1%d^3]^0 %s<< Headers: %s^0'):format(src, callbackId, resultColor, json.encode(headers))
        )
    end
...
RegisterNetEvent('txsv:webpipe:req', function(callbackId, method, path, headers, body)
    ...
    if #path > 500 then
        return SendMenuMessage(s, callbackId, 400, path:sub(1, 300), '{}', {})
    end
    ...
    if not TX_ADMINS[src] then
        ...
        DebugWebPipe(
            string.format(
                '^3WebPipe[^5%d^0:^1%d^3]^0 ^1rejected request from ^3%s^1 for ^5%s^0',
                s,
                callbackId,
                s,
                path
    ...
    if _pipeFastCache[path] ~= nil then
        ...
        SendMenuMessage(s, callbackId, 200, path, cachedData.data, cachedData.headers, true)
        return
    end
    ...
    PerformHttpRequest(url, function(httpCode, data, resultHeaders)
        ...
        SendMenuMessage(s, callbackId, httpCode, path, data, resultHeaders)
